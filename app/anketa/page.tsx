...
function AnketaPageContent() {
  ...
  const isEnvConfigured = useMemo(() => isTelegramConfigured(), []);
  ...
  return (
    <div className="min-h-screen min-h-[100dvh] flex flex-col bg-background overflow-x-hidden">
      ...
      <main className="container mx-auto px-4 py-6 sm:py-8 max-w-2xl w-full min-w-0 flex-1">
        <h1 className="text-2xl sm:text-3xl font-bold text-foreground text-center mb-6 sm:mb-8 animate-fade-in">
          {title}
        </h1>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          {sections.map((section, sectionIndex) => (
            ...
            >
              <h2 className="text-lg sm:text-xl font-semibold text-foreground flex flex-wrap items-center gap-2 min-w-0">
                <SectionIcon name={section.icon} className="w-6 h-6 text-primary shrink-0" />
                <span className="min-w-0">{section.title[language]}</span>
              </h2>
...
export default function AnketaPage() {
  return (
    <Suspense fallback={<AnketaPageFallback />}>
      <AnketaPageContent />
    </Suspense>
  );
}
